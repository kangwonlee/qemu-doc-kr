...
Block device options[¶](#hxtool-1 "Permalink to this headline")
---------------------------------------------------------------
...
```
-blockdev driver=file,node-name=disk\_file,filename=disk.img
-blockdev driver=raw,node-name=disk,file=disk\_file
...
```
-blockdev driver=file,node-name=my\_file,filename=/tmp/disk.qcow2
-blockdev driver=qcow2,node-name=hda,file=my\_file,overlap-check=none,cache-size=16777216
...
`Driver-specific options for other drivers`Please refer to the QAPI documentation of the `blockdev-add`
QMP command.
...
USB convenience options[¶](#hxtool-2 "Permalink to this headline")
------------------------------------------------------------------
...
[Connecting USB devices](devices/usb.html#connecting-usb-devices) in the System Emulation Users Guide.
...
Display options[¶](#hxtool-3 "Permalink to this headline")
----------------------------------------------------------
...
should use `expire\_password <protocol> <expiration-time>`
...
[VNC security](vnc-security.html#vnc-security) section in the System Emulation Users Guide
...
i386 target only[¶](#hxtool-4 "Permalink to this headline")
-----------------------------------------------------------
...
Network options[¶](#hxtool-5 "Permalink to this headline")
----------------------------------------------------------
...
Character device options[¶](#hxtool-6 "Permalink to this headline")
-------------------------------------------------------------------
...
[Keys in the character backend multiplexer](mux-chardev.html#keys-in-the-character-backend-multiplexer) in the
...
`-chardev msmouse,id=id`Forward QEMU’s emulated msmouse events to the guest. `msmouse`
...
`-chardev console,id=id`Send traffic from the guest to QEMU’s standard output. `console`
does not take any options.
...
`-chardev pty,id=id`Create a new pseudo-terminal on the host and connect to it. `pty`
does not take any options.
...
TPM device options[¶](#hxtool-7 "Permalink to this headline")
-------------------------------------------------------------
...
Boot Image or Kernel specific[¶](#hxtool-8 "Permalink to this headline")
------------------------------------------------------------------------
...
Please see the [QEMU System Emulator Targets](targets.html#system-targets-ref) section of the manual for
...
Debug/Expert options[¶](#hxtool-9 "Permalink to this headline")
---------------------------------------------------------------
...
`chardev:id`Use a named character device defined with the `-chardev`
option.
...
Generic object creation[¶](#hxtool-10 "Permalink to this headline")
-------------------------------------------------------------------
...
fw\_cfg. The list is represented as an array of IANA\_TLS\_CIPHER
objects. The firmware uses the IANA\_TLS\_CIPHER array for configuring
guest-side TLS.
...
The [compare\_timeout=@var{ms](mailto:compare_timeout=%40var{ms)} determines the maximum time of the
colo-compare hold the packet. The [expired\_scan\_cycle=@var{ms](mailto:expired_scan_cycle=%40var{ms)}
is to set the period of scanning expired primary node network packets.
The [max\_queue\_size=@var{size](mailto:max_queue_size=%40var{size)} is to set the max compare queue
...
-chardev socket,id=compare\_out,host=3.3.3.3,port=9005,server=on,wait=off
-chardev socket,id=compare\_out0,host=3.3.3.3,port=9005
-object iothread,id=iothread1
-object filter-mirror,id=m0,netdev=hn0,queue=tx,outdev=mirror0
-object filter-redirector,netdev=hn0,id=redire0,queue=rx,indev=compare\_out
-object filter-redirector,netdev=hn0,id=redire1,queue=rx,outdev=compare0
-object colo-compare,id=comp0,primary\_in=compare0-0,secondary\_in=compare1,outdev=compare\_out0,iothread=iothread1
...
-chardev socket,id=compare\_out,host=3.3.3.3,port=9005,server=on,wait=off
-chardev socket,id=compare\_out0,host=3.3.3.3,port=9005
-chardev socket,id=notify\_way,host=3.3.3.3,port=9009,server=on,wait=off
-object filter-mirror,id=m0,netdev=hn0,queue=tx,outdev=mirror0
-object filter-redirector,netdev=hn0,id=redire0,queue=rx,indev=compare\_out
-object filter-redirector,netdev=hn0,id=redire1,queue=rx,outdev=compare0
-object iothread,id=iothread1
-object colo-compare,id=comp0,primary\_in=compare0-0,secondary\_in=compare1,outdev=compare\_out0,notify\_dev=nofity\_way,iothread=iothread1
...
```
account requisite  pam\_listfile.so item=user sense=allow \
...
Device URL Syntax[¶](#device-url-syntax "Permalink to this headline")
---------------------------------------------------------------------
...
```
qemu-system-x86_64 -drive file=iscsi:[//user%password@192.0.2.1/iqn.2001-04.com.example/1](mailto://user%password%40192.0.2.1/iqn.2001-04.com.example/1)
```
...
```
qemu-system-x86_64 -drive file=ssh:[//user@host/path/to/disk.img](mailto://user%40host/path/to/disk.img)
